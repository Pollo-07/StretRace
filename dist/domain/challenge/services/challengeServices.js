...
class challengeServices {
    constructor(challengeRepository, vehicleRepository, userRepository, notificationServicie) {
        this.challengeRepository = challengeRepository;
        this.vehicleRepository = vehicleRepository;
        this.userRepository = userRepository;
        this.notificationServicie = notificationServicie;
    }
    async createchallenge(challenge, notification) {
        const { retado_id, retador_id, vehiculo_retado_id, vehiculo_retador_id } = challenge;
        ...
        const vehicleRetado = await this.vehicleRepository.getVehicle(vehiculo_retado_id);
        const vehicleRetador = await this.vehicleRepository.getVehicle(vehiculo_retador_id);
        const challengeExists = await this.challengeRepository.getMyChallengeExists(retado_id, retador_id);
    ...
    async acceptChallenge(id, id_retado, notification) {
        ...
        if (challenge.estado !== "pendiente")
            throw new Error(`solo se puede aceptar un reto en estado: ${challenge.estado}`);
    ...
    async rejectChallenge(id, id_retado, notification) {
        const challenge = await this.challengeRepository.getchallenge(id);
        if (!challenge)
            throw new Error("no se encontro el challenge");
        const userRetador = await this.userRepository.getUser(challenge.retador_id);
    ...
    async cancelChallenge(id) {
        ...
        if (!["en_curso", "aceptado"].includes(challenge.estado))
            throw new Error(`solo se puede cancelar un reto en estado en curso o aceptado`);
        return await this.challengeRepository.cancelChallenge(id);
    }
    async completeChallenge(id, ganador_id, notification, challengeNotificationIo) {
        try {
    ...
    async reporteChallenge(id, id_ganador, userId, notas, notification) {
        try {
            const challenge = await this.challengeRepository.getchallenge(id);
            if (!challenge)
                throw new Error("no se encontro el challenge");
            ...
            const reporteRetador = reportes.find((r) => r.user_id === challenge.retador_id);
            const reporteRetado = reportes.find((r) => r.user_id === challenge.retado_id);
            if (!reporteRetado || !reporteRetador) {
                ...
                const id_perdedor = id_ganador === challenge.retado_id
                    ...
                    : challenge.retado_id;
                const { ganador, perdedor } = await (0, ranking_1.Ranking)(this.userRepository, id_ganador, id_perdedor);
                await Promise.all([
    ...
    async getChallengeDisputed() {
        return await this.challengeRepository.ChallengeDisputed();
    }
    async resolveChallengeDisputed(id, ganador_id) {
        return await this.challengeRepository.resolveChallengeDisputed(id, ganador_id);
    }
}
exports.default = challengeServices;
//# sourceMappingURL=challengeServices.js.map